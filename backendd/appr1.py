...
def detect_sql_injection(log_df):
    ...
    # Tambahkan definisi ip_regex
    ip_regex = re.compile(r"(\d+\.\d+\.\d+\.\d+)")
    
    log_df['sql_injection_detected'] = log_df['log'].apply(lambda x: bool(sql_regex.search(x)))
    log_df['IP'] = log_df['log'].apply(lambda x: ip_regex.search(x).group() if ip_regex.search(x) else "Unknown")
...
def detect_brute_force(log_df):
    ...
    log_df['brute_force_detected'] = log_df['IP'].apply(lambda x: ip_counts.get(x, 0) > 10 if x != "Unknown" else False)
...
def summarize_results(log_df, dataset_name):
    ...
    for index, row in log_df.iterrows():
        ...
        if attack_type and ip and ip != "Unknown":
            ip_analysis[ip] += 1
...
try:
    while True:
        ...
        total_logs_train = sum(training_results['distribution'])
        total_logs_test = sum(testing_results['distribution'])

        result = {
            "metrics_train": training_results,
            "metrics_test": {
                "total_logs": total_logs_test,
